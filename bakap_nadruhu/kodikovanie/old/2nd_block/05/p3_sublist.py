...
# Složitost může být v nejhorším případě až kubická vzhledem k délce
...
def largest_common_sublist_sum(left: list[int], right: list[int]) -> int:
    ...
    for element_l in left:
        for element_r in right:
            if element_l == element_r:
                final.append(element_l)

    for element in final:
        if element == prev + 1 and prev != 0:
            summary += element
        else:
            summary = element
        prev = element
