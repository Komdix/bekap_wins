...
# žádné nesrovnalosti, a že každý zaměstnanec, který do práce
# přišel, z ní také odešel.
...
def seconds_spent_working(
    ...
    time_entry: dict[str, int] = {}

    for element in records:
        employee, time, in_out = element
        if in_out:
            time_entry[employee] = time
        if not in_out:
            entry = time_entry.get(employee)
            assert entry is not None
            if employee in time_spent:
                time_spent[employee] += time - entry
            else:
                time_spent[employee] = time - entry
            time_entry[employee] = 0
    return time_spent
