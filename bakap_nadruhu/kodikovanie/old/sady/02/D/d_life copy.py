...
def evolution_step(position, state, poison):
    alive = [1, 2]
    dead = [3, 4, 5]
    clear = 6
    neighbors_all = neighbors(position)
    neighbors_alive = sum(1 for element in neighbors_all if state.get(element, 0) in alive)

    main_cell = state.get(position, 0)

    if position in poison or any(element in poison for element in neighbors_all):
        return 3
    ...
    if main_cell in alive:
        ...
        blue = sum(1 for element in neighbors_all if state.get(element, 0) == 1)
        orange = sum(1 for element in neighbors_all if state.get(element, 0) == 2)
...
def evolve(initial, poison, generations):
    ...
    for _ in range(generations):
        next_state = {}
        positions_to_check = set(current.keys()) | {neighbor for pos in current for neighbor in neighbors(pos)}

        for position in positions_to_check:
            new_state = evolution_step(position, current, poison)
