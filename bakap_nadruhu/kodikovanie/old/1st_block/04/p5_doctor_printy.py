...
def all_patients(lst_patients, patients):
    lst = []
    add = True
    for i in range(len(patients)):
        num_of_patient, _ = years_visited(i,patients)
        for j in range(len(lst_patients)):
            # print(lst_patients[j], num_of_patient)
            if lst_patients[j] == num_of_patient:
                add = False
        if add == True:
...
# t = missing_visits(2017,patients)
# print(t)
...
def patient_reports(patients):
    ...
    for i in range(len(patients)):
        ...
        for visit in visits:
            ...
            # print(pulse,out,into)
            if pulse > max_pulse:
                max_pulse = pulse
            if pulse <= prev_pulse and prev_pulse != 2000:
                pulse_check = False
            # print(pulse, prev_pulse)
            prev_pulse = pulse
