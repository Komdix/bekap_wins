...
def distance(a: tuple[float, float, str], b: tuple[float, float, str]) \
        -> float:
    a_x, a_y, _ = a
    b_x, b_y, _ = b
...
def leftmost_colour(points: list[tuple[float, float, str]]) -> str:
...
def center_of_gravity(points: list[tuple[float, float, str]], colour: str)\
...
def average_nonmatching_distance(points: list[tuple[float, float, str]])\
        -> float:
    total = 0.0
    pairs = 0
