...
from types import UnionType  # XXX patch to only allow | None
...
def allow_modules():
    ...
    if level >= 11:
        allow["gzip"] = ["open"]
        allow["http.client"] = ["HTTPSConnection", "HTTPConnection"]
        allow["re"] = ["+findall", "+match", "+compile", "+sub", "+search"]
        allow["zipfile"] = ["ZipFile", "+is_zipfile"]
        allow["io"] = ["BytesIO"]
        allow["glob"] = ["+glob"]
        allow["sys"] = ["stdin", "stdout", "stderr", "argv"]
        allow["csv"] = ["reader", "writer", "DictReader"]
        allow["datetime"] = ["date", "datetime", "timedelta", "time"]
        allow["calendar"] = ["monthrange"]
        allow["json"] = ["+load", "+loads", "+dump", "+dumps"]
        allow["shutil"] = ["+rmtree", "+copyfile"]
        allow["os"] = [
            "path",
            "remove",
            "getcwd",
...
def freeze_globals():
    ...
    with overrides_disabled():
        ...
        for name, obj in gdict.items():
            ...
            if type(obj) in [set, dict, list, tuple, str, int, float]:
