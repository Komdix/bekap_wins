...
class Person:
    ...
    def assigning_the_order(self, alive: set['Person'],
        ...
        index_children = [(child.birth_year, index, child) 
                            for index, child in enumerate(self.children)]

        sorted_children = sorted(index_children)

        for _, _, child in sorted_children:
            if child in alive:
                order[child] = order_num[0]
                order_num[0] += 1
            order = child.assigning_the_order(alive, order, order_num, first)
        return order
    ...
    def remove_extinct_branches(self, alive: set['Person']) -> None:
        for i in range(len(self.children) - 1, -1, -1):
            ...
            if child not in alive and not child.alive_descendant(alive):  # if i dont use this check in the condition it doesnt pop the descendants by DFS
                self.children.pop(i)
