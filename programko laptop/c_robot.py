...
def move_in_direction(x: int, y: int, current_way: int) -> Position:
    directions = [(x, y - 1), (x + 1, y), (x, y + 1), (x - 1, y)]
    return directions[current_way]
...
def is_correct(plan: Plan) -> bool:
    ...
    for element in plan.keys():
        ...
        for way in ways:
            ...
            if move not in plan or (way + 2) % 4 not in plan[move]:
                return False
    return True
...
def run(plan: Plan, start: Position) -> Position:
    x, y = start
    been_to = [start]
    if plan.get(start) == set():
        return start

    start_getter = plan.get(start)
    assert start_getter is not None
    last_way = min(list(start_getter))
    ...
    while pos in plan:
        ...
        for last_way in [last_way, right, left]:
