...
def lngth(grid: Grid) -> int:
    lngth_max = len(max(grid))
    # for element in grid:
    #     lnght_cols = len(element)
    #     if lnght_cols > lngth_max:
    #         lngth_max = lnght_cols
    return lngth_max
...
def diagonals(start_x: int, start_y: int, max_rows: int, max_col: int,
              in_row: int, board: Board, player: str,
              dir_y: int, dir_x: int) -> int:
    in_row = 0
    y = start_y
    x = start_x
    while in_row < 6:
...
def play(grid: Grid, player: str, column: int) -> bool:
    col = grid[column]
    col.append(player)
    max_rows = len(max(grid))

    start_column = len(col)
    board = to_matrix(grid)
    # board_huh = len(max(grid))
    # print(len(board), "should be and alternativ is ", board_huh)
    start_y = max_rows - start_column
    start_x = column

    max_row = max_rows - 1
    max_col = len(board[0]) - 1
    # print(board)
    # print(len(board[0]), "alternative is -" , len(grid))
    ...
    while in_row < 6:
        for element in directions_LR:
            ...
            x = start_x
            dir_x, dir_y = element
            in_row += diagonals(x, y, max_row, max_col, in_row,
                                board, player, dir_y, dir_x)
        if in_row == 4:
            return True
        ...
        for element in directions:
            ...
            while in_row < 6:
                y += dir_y
                x += dir_x
                if y > max_row or y < 0 or x < 0 or x > max_col:
                    break
                row = board[y]
                horizontal = row[x]
