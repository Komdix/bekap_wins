...
def diagonals(start_x, start_y, max_rows, in_row, board, player, dir_y, dir_x):
    in_row = 0
    y = start_y
    while in_row < 4:
        y += dir_y

        if y > max_rows or y < 0:
            break
        col_up = board[y]
        if col_up[start_x + dir_x] == player:
            in_row += 1 
        else:
            break
...
def play(grid: Grid, player: str, column: int) -> bool:
    ...
    while in_row < 4:
        for element in directions_LR:
            ...
            x = start_x
            dir_x, dir_y = element
            in_row += diagonals(x, y, max_rows, in_row, board, player, dir_y, dir_x)
            if in_row > 3:
                return True
        ...
        for element in directions_RL:
            ...
            x = start_x
            dir_x, dir_y = element
            in_row += diagonals(start_x, start_y, max_rows, in_row, board, player, dir_y, dir_x)
            if in_row > 3:
                return True
...
grid = [['O', 'O', 'O'], ['O'], ['X'], ['X'], ['X'], [], ['X']]
s = play(grid, 'X', 5)
print(s)
grid = [['O', 'X', 'X', 'O', 'X', 'O'], ['X', 'O', 'O'], ['O', 'X', 'X'], ['O', 'X', 'O'], ['X']]
print(play(grid, 'X', 1))

#  Mějme: grid = [['O', 'O', 'O'], ['O'], ['X'], ['X'], ['X'], [], ['X']]
#   Test volání: play(grid, 'X', 5)
#   Chyba: Funkce vrátila True, ale měla vrátit False.
# verity test failed       
