...
def dead_step(main_cell: int | None) -> int | None:
    ...
    if main_cell is not None and main_cell in dead:
        return main_cell + 1
...
def evolution_step(position: Position, state: State,
    ...
    if main_cell is not None and main_cell in not_alive:
        return dead_step(main_cell)
    if main_cell is not None and main_cell == clear:
        return None
    ...
    for cell in neighbors_all:
        if position in poison or cell in poison:

            return dead_step(main_cell)

    if main_cell in alive and main_cell is not None:
        if num_neighbors_alive in stayin_alive:
            return main_cell
        else:
            return 3

    if main_cell == 0 or main_cell is None:
...
def evolve(initial: State, poison: set[Position],
    ...
    for _ in range(generations):
        ...
        for positions_around in current.keys():
