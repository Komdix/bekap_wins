...
def masking(index: int, current: str, word: str, mask: str, alternatives: list, results: set):
    """ Recursive function to apply mask and alternatives """
    if index == len(word):
        results.add(current)
        return
    ...
    if mask_char == 'X':
        masking(index + 1, current + word[index], word, mask, alternatives, results)
    elif mask_char == '?':
        for alt in alternatives:
            masking(index + 1, current + alt, word, mask, alternatives, results)

def wordmask(word: str, mask: str, alternatives: list) -> list:
    results = set()
