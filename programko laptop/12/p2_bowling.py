...
def bowling_score(rolls: list[int]) -> int:
    ...
    lngth = len(rolls)
    last = lngth - 1
    last_two = [lngth - 2, last]

    i = -1
    pair = 0

    for current in rolls:
        pair += 1
        i += 1

        if current == 10 and (i not in last_two):
            points += current + rolls[i + 1] + rolls[i + 2]
            round_points = 0
            pair = 0
            continue
        ...
        if pair == 2 and round_points < 10:
            points += round_points
            pair = 0
            round_points = 0

        if pair == 2 and round_points == 10 and i != last:
            round_points += rolls[i + 1]
            points += round_points
            round_points = 0
            pair = 0
