...
def to_matrix(grid: Grid) -> list[list[str]]:
    rows = max(len(elem) for elem in grid)
    columns = []
    lst: list[str] = []
    lst_maxtrix = []
...
def control_in_row(start: tuple[int], bounds: tuple[int],
...
def play(grid: Grid, player: str, column: int) -> bool:
    ...
    max_rows = max(len(elem) for elem in grid)
    board = to_matrix(grid)
    # print(board)
    # # print(grid)
    start_y = max_rows - start_row
    # print(start_y, len(grid[column]), start_row)
    start_x = column
    ...
    while in_row < 6:
        ...
        for direction in directions:
            if pair == 2:  # I need to control 2 ways from the point of added move
...
# # print(max(len(elem) for elem in grid))
# # print(grid)
# # play(grid, 'X', 2)
# # print(to_matrix(grid))
# [['O', ' ', 'X', 'O', ' ', ' '], 
#  ['O', 'X', 'O', 'O', ' ', ' '], 
#  ['X', 'O', 'X', 'O', ' ', ' ']]
