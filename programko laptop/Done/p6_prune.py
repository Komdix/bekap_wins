...
def prune_subtree(node: Node | None, keep: set[int]) -> Node | None:
    if node is None or node.value not in keep:
        return None

    node.left = prune_subtree(node.left, keep)
    node.right = prune_subtree(node.right, keep)

    return node


def prune(tree: Tree, keep: set[int]) -> None:
    tree.root = prune_subtree(tree.root, keep)
