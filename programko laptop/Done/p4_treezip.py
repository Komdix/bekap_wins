...
# Napište (čistou) funkci, která na obdrží jednu instanci ‹IntTree›
...
def treezip(it: IntTree, st: StrTree) -> TupleTree:
    if it is None and st is None:
        return None

    new = TupleTree((it.value, st.value))

    new.left = None
    new.right = None

    if it.left is not None and st.left is not None:
        new.left = treezip(it.left, st.left)
    if it.right is not None and st.right is not None:
        new.right = treezip(it.right, st.right)
